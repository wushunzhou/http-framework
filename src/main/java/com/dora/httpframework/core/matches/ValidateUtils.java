...
import com.dora.httpframework.core.config.base.ConstantConfig;
import com.dora.httpframework.utils.ReflectUtil;
...
public class ValidateUtils {
    ...
    public static boolean plugin(Map<String, Object> operand) {
        ...
        if (operand.get(METHOD_ARGS_KEY) instanceof String) {
            invoke = ReflectUtil.reflectInvoke(ConstantConfig.PLUGIN_METHOD_BY_CLASS, (String) operand.get(METHOD_NAME_KEY), (String) operand.get(METHOD_ARGS_KEY));
        } else if (operand.get(METHOD_ARGS_KEY) instanceof Object[]) {
            Object[] args = (Object[]) operand.get(METHOD_ARGS_KEY);
            invoke = ReflectUtil.reflectInvoke(ConstantConfig.PLUGIN_METHOD_BY_CLASS, (String) operand.get(METHOD_NAME_KEY), args);
        } else {
            invoke = ReflectUtil.reflectInvoke(ConstantConfig.PLUGIN_METHOD_BY_CLASS, (String) operand.get(METHOD_NAME_KEY), "");
        }
    ...
    public static boolean plugin(String method, Object...args) {
        Object invoke = ReflectUtil.reflectInvoke(ConstantConfig.PLUGIN_METHOD_BY_CLASS, method, args);
        if (invoke != null && invoke instanceof Boolean) {
            return (boolean) invoke;
        }
        return true;
