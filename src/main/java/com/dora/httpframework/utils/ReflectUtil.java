...
public class ReflectUtil {
    ...
    public static <T> Object reflectInvoke(Class<T> clazz, String methodName, Object[] args) {
        try {
            ...
            T obj = SpringContext.getBean(clazz) != null ? SpringContext.getBean(clazz) : clazz.newInstance();
                ...
                if (methodName.equals(method.getName())) {
                    ...
                    for (int i = 0; i < parameterTypes.length; i++) {
                        if (args[i] instanceof String) {
                            String arg = Parameterization.wildcardMatcherString((String) args[i]);
                            arg = arg.trim();
                            ...
                            if ("String".equals(simpleName)) {
                                ...
                                throw new DoraException("%s类,方法: %s,入参类型: %s,不支持该类型", clazz.getName(), methodName, simpleName);
                            }
                        } else {
                            invokeArgs[i] = args[i];
                        }
                    ...
                    try {
                        ...
                        if (e.getTargetException() instanceof DoraException){
                            throw (DoraException) e.getTargetException();
                        }
                        throw new DoraException("%s类, 方法: %s,异常: %s", clazz.getName(), methodName, e.getTargetException());
                    } catch (IllegalArgumentException e) {
                        throw new DoraException("%s类, 方法: %s,参数不匹配", clazz.getName(), methodName);
                    }
                }
            }
            if (!isMethodExist) {
                throw new DoraException("%s类, 方法: %s未找到", clazz.getName(), methodName);
            }
        } catch (InstantiationException e) {
            throw new DoraException("%s, 实例化异常,该类是一个抽象类或接口", clazz.getName());
        } catch (IllegalAccessException e) {
            throw new DoraException("%s,非法访问异常,请设置setAccessible(true)", clazz.getName());
        }
        return null;
    }
